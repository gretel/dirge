...
pub struct Config {
    ...
    /// Working-context budget in tokens (default: no cap — the model's
    /// advertised window is used as-is). The compaction decision treats the
    /// effective window as `min(model_window, this)`, so setting e.g. 100_000
    /// caps the live context to fold — and form memory — early, staying
    /// within a tighter budget instead of trusting the model's full window.
    /// Floored at 16k; a value above the model's real window is a no-op (the
    /// window wins). Installed process-wide at startup.
